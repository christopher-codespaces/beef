...
export default function Favs() {
  const supabaseUrl = 'https://psntehsynbqawexkqfds.supabase.co';
  const supabaseKey  = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBzbnRlaHN5bmJxYXdleGtxZmRzIiwicm9sZSI6ImFub24iLCJpYXQiOjE2OTA1MzEwMDYsImV4cCI6MjAwNjEwNzAwNn0.4llF_lTi2IU5bVQsFtpMOQqI5oBc5giImBEk4TXk6us';
  ...
  const toggleFavorite = async (podcastId) => {
    if (favoritePodcastIds.includes(podcastId)) {
      ...
      // Remove the favorite podcast ID from the Supabase table
      await supabase
        .from('favorites')
        .delete()
      ...
      // Add the favorite podcast ID to the Supabase table
      await supabase.from('favorites').insert([{ podcast_id: podcastId }]);
    }
  };
